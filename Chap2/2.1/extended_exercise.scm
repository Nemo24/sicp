...
;One feature she wants to provide in her system is the ability to manipulate 
;inexact quantities (such as measured parameters of physical devices) 
;with known precision, so that when computations are done with such 
...
; the value of a parallel equivalent resistance Rp of 
...
;a resistor labeled ``6.8 ohms with 10% tolerance'' 
;you can only be sure that the resistor has a resistance 
;between 6.8 - 0.68 = 6.12 and 6.8 + 0.68 = 7.48 ohms. 
;Thus, if you have a 6.8-ohm 10% resistor in parallel 
;with a 4.7-ohm 5% resistor, the resistance of the 
;combination can range from about 2.58 ohms 
;(if the two resistors are at the lower bounds)
